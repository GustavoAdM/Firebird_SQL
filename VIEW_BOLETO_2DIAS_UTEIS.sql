...
/* View: BOLETO_2DIAS_UTEIS */
CREATE OR ALTER VIEW VIEW_BOLETO_2DIAS_UTEIS(
    CD_PESSOA,
    NM_PESSOA,
    ST_CONTAS,
    DT_VENC,
...
WHERE C.ST_CONTAS NOT IN ('C','L','A')
    AND C.DT_VENCIMENTO = (CASE EXTRACT(WEEKDAY FROM CURRENT_DATE)
                                WHEN 2 THEN CURRENT_DATE - 3
                                WHEN 3 THEN CURRENT_DATE - 3
                                END)
...
/* Privileges of users */
GRANT ALL ON VIEW_BOLETO_2DIAS_UTEIS TO JUNSOFT WITH GRANT OPTION;